...
def train_agent_single_process(args: Config):
    ...
    debug_train_loop = _env_flag("ERL_TRAIN_DEBUG")
    train_loop_iter = 0
    ...
    while if_train:
        ...
        if debug_train_loop:
            print(
                f"[ERL_TRAIN_DEBUG] stage=explore_start loop={train_loop_iter} horizon_len={horizon_len} num_envs={env.num_envs}",
                flush=True,
            )
        explore_t0 = time.time()
        buffer_items = agent.explore_env(env, horizon_len)
        explore_dt = time.time() - explore_t0
        if debug_train_loop:
            reward_index = 3 if not if_off_policy else 2
            reward_tensor = buffer_items[reward_index]
            print(
                "[ERL_TRAIN_DEBUG] "
                f"stage=explore_done loop={train_loop_iter} seconds={explore_dt:.3f} "
                f"reward_mean={reward_tensor.mean().item():.6g} reward_std={reward_tensor.std(unbiased=False).item():.6g}",
                flush=True,
            )
        """buffer_items
        ...
        if if_discrete:
            ...
            show_str = ''
        exp_r = buffer_items[3 if not if_off_policy else 2].mean().item()

        th.set_grad_enabled(True)
        if debug_train_loop:
            print(
                f"[ERL_TRAIN_DEBUG] stage=update_start loop={train_loop_iter}",
                flush=True,
            )
        update_t0 = time.time()
        ...
        if debug_train_loop:
            print(
                "[ERL_TRAIN_DEBUG] "
                f"stage=update_done loop={train_loop_iter} seconds={update_dt:.3f} "
                f"logging_tuple={logging_tuple}",
                flush=True,
            )
...
class Learner(Process):
    ...
    def run(self):
        ...
        while if_train:
            ...
            if if_train:
                exp_r = buffer_items_tensor[3 if not if_off_policy else 2].mean().item()  # avg reward of exploration
