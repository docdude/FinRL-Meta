    ...
    results_log.append(
        {
            ...
            "benchmark_value": bench_val.iloc[env.time],
            "cash": env.cash,
        }
    )
...
def train_with_epoch_evaluation(
    ...
    for epoch in range(num_epochs):
        ...
        # Train eval
        ev_tr = MarginTraderImpactEnv(
            config=train_config, impact_model=impact_model_class(), **env_kwargs
        )
        r_tr, t_tr = run_and_log_simulation(
            ev_tr, model, train_config["date_list"], train_bench
        )
        ts = compute_performance_stats(
            r_tr["portfolio_value"],
            r_tr["turnover"],
            r_tr["cost"],
            t_tr,
            rewards=r_tr["reward"],
        ...
        ev_te = MarginTraderImpactEnv(
            config=trade_config, impact_model=impact_model_class(), **env_kwargs
        )
        r_te, t_te = run_and_log_simulation(
            ev_te, model, trade_config["date_list"], trade_bench
        )
        tt = compute_performance_stats(
            r_te["portfolio_value"],
            r_te["turnover"],
            r_te["cost"],
            t_te,
            rewards=r_te["reward"],
        ...
        log.info(
            f"  Train Sharpe={ts['annualized_sharpe']:.3f} | Test Sharpe={tt['annualized_sharpe']:.3f}"
        )
...
def train_and_backtest(data_prep, configs, num_epochs=20) -> str:
    ...
    for idx, cfg in enumerate(configs, 1):
        ...
        mdl = DRLAgent(env=vec).get_model(
            mn, model_kwargs=mk, policy_kwargs=pk, seed=42
        )
        ...
        r_is, t_is = run_and_log_simulation(
            tr_env, trained, train_cfg["date_list"], tr_bench
        )
        ...
        te_env = MarginTraderImpactEnv(
            config=trade_cfg,
            impact_model=imp,
            initial_margin_state=margin_state,
            **ek,
        )
        ...
        r_oos, t_oos = run_and_log_simulation(
            ...
            initial_benchmark_value=last_train_bench,
        )
        r_oos.to_csv(f"{results_dir}/{base}_test.csv", index=False)
        t_oos.to_csv(f"{results_dir}/{base}_test_trades.csv", index=False)

        # OOS blank slate
        te_env_blank = MarginTraderImpactEnv(
            config=trade_cfg, impact_model=imp_cls(), **ek
        )
        ...
        all_meta.append(
            {
                ...
                "model_kwargs": mk,
                "policy_kwargs": pk,
                "epoch_stats_train": es_tr,
                "epoch_stats_test_blank": es_te,
            }
        )
    ...
    with open(summary, "w") as f:
        json.dump(
            {"benchmark_ticker": data_prep.benchmark_ticker, "backtests": all_meta},
            f,
            indent=4,
            default=str,
        )
...
def run_example():
    ...
    log.info(
        f"Generated {len(configs)} configs ({len(algorithms)} algos × {len(impact_models)} impact models)"
    )
