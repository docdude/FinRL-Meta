...
def train_with_epoch_evaluation(
    ...
    for epoch in range(num_epochs):
        ...
        ts = compute_performance_stats(
            res_train["portfolio_value"],
            res_train["turnover"],
            res_train["cost"],
            trades_train,
            rewards=res_train["reward"],
        )
        ts["epoch"] = epoch + 1
        ...
        tt = compute_performance_stats(
            res_test["portfolio_value"],
            res_test["turnover"],
            res_test["cost"],
            trades_test,
            rewards=res_test["reward"],
        )
        tt["epoch"] = epoch + 1
...
def train_and_backtest(
    ...
    for idx, cfg in enumerate(configs, 1):
        ...
        mdl = agent.get_model(
            model_name, model_kwargs=eff_mk, policy_kwargs=pk, seed=42
        )
...
def run_example():
    ...
    _hpo_agent_params = {
        "a2c": {  # OOS Sharpe=1.1703, trial #87
            "env_kwargs_extra": {
                "reward_scaling": 8.0107,
                "max_trade_volume_pct": 0.1934,
            },
            "model_kwargs": {
                "learning_rate": 8.428e-4,
                "n_steps": 10,
                "ent_coef": 1.961e-4,
                "gamma": 0.9535,
                "gae_lambda": 0.8869,
        ...
        "ppo": {  # OOS Sharpe=1.2065, trial #89
            "env_kwargs_extra": {
                "reward_scaling": 0.1009,
                "max_trade_volume_pct": 0.1210,
            },
            "model_kwargs": {
                "learning_rate": 2.111e-3,
                "n_steps": 1024,
                "batch_size": 128,
                "ent_coef": 2.154e-4,
                "gamma": 0.9550,
        ...
        "ddpg": {  # OOS Sharpe=1.1046, trial #93
            "env_kwargs_extra": {
                "reward_scaling": 0.1623,
                "max_trade_volume_pct": 0.1400,
            },
            "model_kwargs": {
                "learning_rate": 1.619e-5,
                "batch_size": 256,
                "buffer_size": 10_000,
                "gamma": 0.9976,
                "tau": 3.459e-3,
        ...
        "sac": {  # OOS Sharpe=1.1716, trial #80
            "env_kwargs_extra": {
                "reward_scaling": 1.3005,
                "max_trade_volume_pct": 0.0971,
            },
            "model_kwargs": {
                "learning_rate": 1.033e-3,
                "batch_size": 128,
                "buffer_size": 50_000,
                "gamma": 0.9820,
                "tau": 7.623e-3,
        ...
        "td3": {  # OOS Sharpe=1.1451, trial #41
            "env_kwargs_extra": {
                "reward_scaling": 0.1197,
                "max_trade_volume_pct": 0.1440,
            },
            "model_kwargs": {
                "learning_rate": 4.053e-5,
                "batch_size": 64,
                "buffer_size": 50_000,
                "gamma": 0.9487,
                "tau": 1.251e-2,
