"""Hyperparameter optimisation for DRL agents using the MarginTraderImpactEnv.

Searches over both environment parameters (margin_rate, maintenance_margin,
...
from meta.env_market_impact.example_margin_trader import run_and_log_simulation
...
def sample_env_kwargs(trial: optuna.Trial, initial_capital: float) -> dict:
...
def objective(
    ...
    *,
    ...
    train_env = MarginTraderImpactEnv(
    ...
    for epoch in range(num_epochs):
        ...
        eval_env = MarginTraderImpactEnv(
...
def run_multi_agent_hpo():
    ...
    results_dir = f"hpo_results/margin_trader_{ts}"
    os.makedirs(results_dir, exist_ok=True)

    for mn in agents:
        log.info(f"{'='*60}\nHPO: {mn.upper()} (Margin Trader)\n{'='*60}")
        try:
