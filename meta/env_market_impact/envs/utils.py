...
def _compute_performance_stats_core(
    portfolio_values,
    turnovers,
    costs,
    *,
    avg_order_pov: float = 0.0,
    wtd_avg_turnover_percentile: float = 0.0,
    rewards=None,
) -> dict:
...
def compute_performance_stats_from_aggregates(
    portfolio_values,
    turnovers,
    costs,
    *,
    ...
    turnover_percentile_notional_sum: float = 0.0,
    rewards=None,
) -> dict:
    avg_order_pov = (pov_sum / trade_count) * 100 if trade_count > 0 else 0.0
    if total_notional > 0:
        wtd_avg_turnover_percentile = (
            turnover_percentile_notional_sum / total_notional
        )
    else:
        wtd_avg_turnover_percentile = 0.0

    return _compute_performance_stats_core(
        portfolio_values,
        turnovers,
        costs,
        avg_order_pov=avg_order_pov,
        wtd_avg_turnover_percentile=wtd_avg_turnover_percentile,
        rewards=rewards,
    )
...
def compute_performance_stats(
    ...
    if trades_df is not None and not trades_df.empty:
        ...
        pov_sum = float(trades_df["pov"].sum())
        total_notional = float(trades_df["notional"].sum())
        turnover_percentile_notional_sum = float(
